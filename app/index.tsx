...
useEffect(() => {
    const handleDeepLink = async (url: string | null) => {
      ...
      if (error) {
        Toast.show({ type: 'error', text1: 'Confirmation failed', text2: error.message });
      } else {
        Toast.show({ type: 'success', text1: 'Email Verified', text2: 'Your email was verified successfully', position: 'top' });
        // Redirect to auth page (index)
        router.replace('/');
      }
    };
    ...
    // Auth state listener - detects SIGNED_IN after confirmation
    const { data: listener } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        const wasUnconfirmed = previousSessionRef.current?.user?.id === session.user.id &&
                               !previousSessionRef.current.user.confirmed_at;

        if (wasUnconfirmed && session.user.confirmed_at) {
          Toast.show({
            type: 'success',
            text1: 'Email Confirmed!',
            text2: 'You have confirmed your email',
            position: 'top',
          });
    ...
    try {
      if (mode === 'sign-up') {
        const { data: { user }, error: signUpError } = await supabase.auth.signUp({
          ...
          password,
          options: { 
          emailRedirectTo: 'exp://192.168.1.134:8081/home', 
            data: { 
  ...
  const handleSendReset = async () => {
    ...
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(forgotEmail, {
        redirectTo: 'localhost:8081/reset-password',
      });
